...
void MetaServiceImpl::prepare_index(::google::protobuf::RpcController* controller,
    ...
    if (is_table_stream) {
        // Review 导读 - Create 1/3（MS）：RecycleIndexPB 中的 PREPARED 是临时创建标记。
        // 已有相同 PREPARED 时直接幂等成功；PREPARED 已消失但 Offset 已存在时，
        // 说明非空 Stream 的创建流程已经完成。
...
void MetaServiceImpl::commit_index(::google::protobuf::RpcController* controller,
    ...
    if (is_table_stream) {
        // Review 导读 - Create 3/3（MS）：commit_index 只负责结束临时创建状态。
        // Stream 的存在性和绑定关系由 FE Catalog 保存，因此这里不写 Stream Mapping，
        // 也不写物理 Index 元数据。
        MultiVersionStatus multi_version_status;
        TableStreamReadResult status_result = read_table_stream_multi_version_status(
                txn.get(), instance_id, TableStreamReadIntent::CONFLICT, &multi_version_status);
...
void MetaServiceImpl::drop_index(::google::protobuf::RpcController* controller,
                                 const IndexRequest* request, IndexResponse* response,
                                 ::google::protobuf::Closure* done) {
    // Review 导读 - Delete（MS）：TABLE_STREAM 只复用 DropIndexLogPB 和 RecycleIndexPB
    // 持久化异步回收任务；没有 Stream Mapping 或 Tablet 元数据需要删除。
...
void MetaServiceImpl::commit_partition_internal(const PartitionRequest* request,
    ...
    if (request->object_type() == IndexObjectTypePB::TABLE_STREAM) {
        // Review 导读 - Create 2/3（MS）：按分区初始化 Latest Offset，并在需要时写入
        // Versioned Offset。commit_index 完成后的重试只确认本批 Offset 都已存在，
        // 绝不覆盖后续消费已经推进过的值。
        MultiVersionStatus multi_version_status;
        TableStreamReadResult status_result = read_table_stream_multi_version_status(
                txn.get(), instance_id, TableStreamReadIntent::CONFLICT, &multi_version_status);
        ...
        if (!create_in_progress) {
            ...
            for (int64_t partition_id : partition_ids) {
                if (stream_it == committed_offsets.end() ||
                    !stream_it->second.contains(partition_id)) {
                    code = MetaServiceCode::INVALID_ARGUMENT;
                    msg = fmt::format(
                            "table stream create is not in progress and offset {} is missing",
                            partition_id);
                    return;
                }
            }
