...
static bool process_table_stream_updates(Transaction* txn, const CommitTxnRequest* request,
                                         ...
                                         std::string& msg) {
    // Review 导读 - Update（MS）：通过 conflict read 校验 expected Offset，并在目标表
    // 提交使用的同一个 immediate FDB transaction 中写入 next Offset。source VersionPB
    // 只使用 snapshot read 校验 TSO 上界。
