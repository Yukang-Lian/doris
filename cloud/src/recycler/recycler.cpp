...
int InstanceRecycler::recycle_stream(int64_t stream_id, const RecycleIndexPB& recycle_index,
                                     std::string_view recycle_key) {
    // Review 导读 - Delete（Recycler）：先删除该 Stream 的 Latest/Versioned Offset
    // 前缀，再删除 RecycleIndexPB 任务。本控制面回收不处理 Tablet、Index Mapping
    // 或 Row Binlog 数据。
...
int InstanceRecycler::recycle_partition_table_stream_offsets(
    ...
    // Review 导读 - Delete Partition：FE 在分区消失前枚举关联 Stream identity，
    // DropPartitionLogPB/RecyclePartitionPB 将这份确定列表传递到这里。
